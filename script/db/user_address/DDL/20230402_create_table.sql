...
COMMENT ON COLUMN cs_user_address.started_at         IS 'Start datatime where live in this address';
COMMENT ON COLUMN cs_user_address.ended_at           IS 'End datatime where live in this address';
COMMENT ON COLUMN cs_user_address.cs_user_id         IS 'FK from cs_user table';
COMMENT ON COLUMN cs_user_address.enabled 			 IS 'if it is 1 the record is active, otherwise it is disable';
COMMENT ON COLUMN cs_user_address.created_at 		 IS 'Log time when created record on the table';
COMMENT ON COLUMN cs_user_address.updated_at 		 IS 'Log time when updated record on the table';
